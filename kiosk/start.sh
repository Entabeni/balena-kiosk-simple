...
echo "chromium-browser $CHROME_LAUNCH_URL $FLAGS --incognito --noerrdialogs --disable-translate  --disable-infobars --disk-cache-dir=/dev/null  --password-store=none --disable-pinch --disable-features=TranslateUI --disable-session-crashed-bubble   --window-size=$WINDOW_SIZE --window-position=$WINDOW_POSITION $OUTPUT" >> /home/chromium/xstart.sh
